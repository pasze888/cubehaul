...
func TestCfSortQuery(t *testing.T) {
	tests := []struct {
		...
		{"explicit sort wins over default", SearchOptions{Query: "sodium", Sort: "downloads"}, 6, ""},
		{"downloads keeps direction unset", SearchOptions{Sort: "downloads"}, 6, ""},
		...
		{"sort-order applies to other fields", SearchOptions{Query: "sodium", Sort: "downloads", SortOrder: "desc"}, 6, "desc"},
		{"case insensitive sort", SearchOptions{Sort: "Popularity"}, 2, ""},
